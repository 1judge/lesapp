  ...
  return (
    <div className="space-y-6">
      <section className="card p-4">
        <div className="text-sm text-[var(--text-secondary)]">{fixture.league?.name} • {new Date(fixture.fixture?.date).toLocaleString()}</div>
      ...
      {prediction ? (
        <section className="grid gap-4 md:grid-cols-2">
          <div className="card p-4">
            ...
            {/* @ts-expect-error Async Server/Client interop */}
            {(await (async () => {
              const Live = (await import('@/components/match/LiveScoreDisplay')).LiveScoreDisplay;
              return <Live fixtureId={fixture.fixture.id} initial={fixture} />;
            })())}
          </div>
          <div className="card p-4">
          ...
          <div className="card p-4">
            ...
            <div className="mt-2 text-sm text-[var(--text-secondary)]">Expected Total: <span className="text-white font-semibold">{prediction.goals.expectedTotal}</span></div>
            <div className="mt-4 grid grid-cols-2 gap-3">
              {(['0.5','1.5','2.5','3.5'] as const).map((line) => (
                <div key={line} className="card p-3">
                  ...
                  <div className="mt-2 text-xs text-[var(--text-secondary)]">Over {prediction.goals.overUnder[line].over}% • Under {prediction.goals.overUnder[line].under}%</div>
                </div>
              ))}
            </div>
            <div className="mt-4">
          ...
          <div className="card p-4">
            ...
            <div className="mt-2 text-sm text-[var(--text-secondary)]">Expected Total: <span className="text-white font-semibold">{prediction.corners.expectedTotal}</span></div>
            <div className="mt-3 grid grid-cols-3 gap-2 text-xs text-[var(--text-secondary)]">
              {(['8.5','9.5','10.5'] as const).map((line) => (
                <div key={line} className="card p-2"><div className="text-white">O/U {line}</div><div>O {prediction.corners.overUnder[line].over}% • U {prediction.corners.overUnder[line].under}%</div></div>
              ))}
            </div>
          </div>
          ...
          <div className="card p-4">
            ...
            <div className="mt-2 text-sm text-[var(--text-secondary)]">Expected Yellow: <span className="text-white font-semibold">{prediction.cards.expectedYellow}</span> • Expected Red: <span className="text-white font-semibold">{prediction.cards.expectedRed}</span></div>
            <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-[var(--text-secondary)]">
              {(['3.5','4.5'] as const).map((line) => (
                <div key={line} className="card p-2"><div className="text-white">O/U {line}</div><div>O {prediction.cards.overUnder[line].over}% • U {prediction.cards.overUnder[line].under}%</div></div>
              ))}
            </div>
          </div>
          ...
          {(await (async () => {
            const Stats = (await import('@/components/match/StatsComparison')).StatsComparison;
            return <Stats homeId={fixture.teams.home.id} awayId={fixture.teams.away.id} leagueId={fixture.league.id} season={fixture.league.season} />;
          })())}

          {/* @ts-expect-error Async Server Component */}
          {(await (async () => {
            const Form = (await import('@/components/match/FormGuide')).FormGuide;
            return <Form homeId={fixture.teams.home.id} awayId={fixture.teams.away.id} season={fixture.league.season} />;
          })())}

          {/* @ts-expect-error Async Server Component */}
          {(await (async () => {
            const H2H = (await import('@/components/match/HeadToHeadTable')).HeadToHeadTable;
            return <H2H homeId={fixture.teams.home.id} awayId={fixture.teams.away.id} />;
          })())}
