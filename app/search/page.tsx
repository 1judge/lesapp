          ...
          <div>
            ...
            <div className="grid gap-2">
              {data.teams?.map((t: any, i: number) => (
                <a key={i} href={`/team/${t.team.id}`} className="card p-3 text-white hover:bg-[var(--surface-hover)] transition">{t.team.name}</a>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="grid gap-2">
              {data.leagues?.map((l: any, i: number) => (
                <a key={i} href={`/leagues/${l.league.id}`} className="card p-3 text-white hover:bg-[var(--surface-hover)] transition">{l.league.name}</a>
              ))}
            </div>
          </div>
        </div>
